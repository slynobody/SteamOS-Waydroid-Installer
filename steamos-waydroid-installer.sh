...
echo SteamOS Waydroid Installer Script by ryanrudolf / adapted for MAIN-channel / slynobody
sleep 2
...
echo -e "$current_password\n" | sudo -S pacman -S --noconfirm fakeroot debugedit dkms plymouth \
	linux-neptune-$(uname -r | cut -d "-" -f5)-headers
	#--overwrite "*" &> /dev/null
...
# binder compilation gives error, but no fatal error (anbox seems to be enough; maybe indication valve waydroid is around the edge)
#                                                     #############################################################################
#if [ $? -eq 0 ]
#then
...
echo -e "$current_password\n" | sudo -S pacman -S --noconfirm cage wlr-randr > /dev/null
...
then
	...
	Choice=$(zenity --width 1040 --height 320 --list --radiolist --multiple \
		...
		TRUE TV13_NO_GAPPS "Download unofficial Android 13 TV image without Google Play Store - thanks SupeChicken666 for the build instructions!" \
