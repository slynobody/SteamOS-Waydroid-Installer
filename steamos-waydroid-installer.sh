...
kernel6=6.8.12-valve7-1-neptune-68
...
if [ $kernel_version = $kernel1 ] || [ $kernel_version = $kernel2 ] || [ $kernel_version = $kernel3 ] || [ $kernel_version = $kernel4 ] || [ $kernel_version = $kernel5 ] || [ $kernel_version = $kernel6 ] || [ $kernel_version = $kernel7 ]  || [ $kernel_version = $kernel8 ] || [ $kernel_version = $stable_kernel1 ] || [ $kernel_version = $stable_kernel2 ] || [ $kernel_version = $beta_kernel1 ]
...
# sanity check - make sure there is enough free space in the home partition (at least 3GB)
echo Checking if home partition has enough free space
echo home partition has $FREE_HOME free space.
if [ $FREE_HOME -ge 3000000 ]
then
...
# dnsmasq + lxc seem not to be included from self-compiled sources now
#echo -e "$current_password\n" | sudo -S pacman -S dnsmasq lxc wlroots

#install packages exclusively compiled for 3.7+
